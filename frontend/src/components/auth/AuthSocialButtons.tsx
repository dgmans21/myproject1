"use client";

import { useState } from "react";
import { AUTH_SOCIAL_PROVIDERS, type AuthSocialProvider } from "@/lib/auth-ui-constants";
import { cn } from "@/lib/utils";

interface AuthSocialButtonsProps {
  disabled?: boolean;
  /** Supabase 연동 전 UI 미리보기용 */
  onProviderClick?: (provider: AuthSocialProvider) => void;
}

export function AuthSocialButtons({ disabled, onProviderClick }: AuthSocialButtonsProps) {
  const [pending, setPending] = useState<AuthSocialProvider | null>(null);

  const handleClick = async (provider: AuthSocialProvider) => {
    if (disabled || pending) return;
    setPending(provider);
    try {
      if (onProviderClick) {
        onProviderClick(provider);
      } else {
        // TODO: supabase.auth.signInWithOAuth({ provider })
        await new Promise((r) => setTimeout(r, 400));
        alert(
          `${AUTH_SOCIAL_PROVIDERS.find((p) => p.id === provider)?.label}\n\nSupabase Auth 연동 후 동작합니다.`
        );
      }
    } finally {
      setPending(null);
    }
  };

  return (
    <div className="space-y-2">
      <p className="text-center text-xs font-medium uppercase tracking-wide text-muted">
        소셜 로그인
      </p>
      {AUTH_SOCIAL_PROVIDERS.map((provider) => (
        <button
          key={provider.id}
          type="button"
          disabled={disabled || Boolean(pending)}
          onClick={() => handleClick(provider.id)}
          className={cn(
            "flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-all active:scale-[0.98] disabled:opacity-50",
            provider.className
          )}
        >
          <SocialIcon provider={provider.id} />
          {pending === provider.id ? "연결 중…" : provider.label}
        </button>
      ))}
    </div>
  );
}

function SocialIcon({ provider }: { provider: AuthSocialProvider }) {
  if (provider === "google") {
    return (
      <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden>
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
          fill="#EA4335"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>
    );
  }
  if (provider === "kakao") {
    return (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
        <path d="M12 3c-5.52 0-10 3.58-10 8 0 2.84 1.78 5.34 4.45 6.78-.15.54-.97 3.47-1 3.66 0 .06.03.12.08.15.05.03.11.03.16 0 .07-.04 3.38-2.24 4.7-3.12.53.08 1.07.12 1.61.12 5.52 0 10-3.58 10-8s-4.48-8-10-8z" />
      </svg>
    );
  }
  // naver 보류
  // return (
  //   <span className="text-base font-bold leading-none" aria-hidden>
  //     N
  //   </span>
  // );
  return null;
}
